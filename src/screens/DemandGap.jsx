...
import { num, pct } from "../data/format";
...
export default function DemandGap() {
  ...
  const gapBars = [
    { label: "Mon/Fri afternoon (baseline)", value: 1, tone: "#94A3B8" },
    { label: "Tue–Thu 2–5pm (actual)", value: gap.own_baseline_ratio, tone: "#ED1C24" },
    { label: "Nearby cafés, same window", value: gap.district_peer_median_ratio, tone: "#1E293B" },
  ];
  ...
  return (
    <div className="max-w-container mx-auto px-6 py-10">
      <SectionTitle
        ...
        subtitle={`${merchant.canonical_name} sees every card that taps at its own counter. It has no way to see the ${num(segment.size)} people who tap at a café just like it, two streets over.`}
      />
      ...
      <Card className="p-8 mb-6 border-brand/20 bg-gradient-to-br from-white to-[#FFF8F8]">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
            ...
            <div className="font-num text-[52px] font-extrabold text-brand leading-none">{num(segment.size)}</div>
            <div className="text-[14px] text-ink-secondary mt-1">
              cardholders who regularly shop at{" "}
              <span className="font-semibold text-ink">{rival.canonical_name}</span> — a comparable café — and have{" "}
              <span className="font-semibold text-ink">zero recorded visits</span> to {merchant.canonical_name}.
            </div>
          </div>
        ...
        <div className="md:col-span-2">
          <Card className="p-5 h-full">
            ...
            <p className="text-[12px] text-ink-secondary mb-3">
              Context for the gap above — Tuesday–Thursday 2–5pm specifically underperforms, both against{" "}
              {merchant.canonical_name}'s own week and against nearby cafés in the same window.
            </p>
            ...
            <BasisNote>
              {pct(Math.abs(gap.magnitude_vs_own_baseline_pct) / 100)} below own weekday-afternoon baseline · {pct(Math.abs(gap.magnitude_vs_district_peers_pct) / 100)} below district café peers
              (demand_gaps.json, confidence: {gap.confidence}).
            </BasisNote>
          </Card>
        </div>
        <Card className="p-5">
          ...
          <p className="text-[11px] text-ink-light mt-3">
            Shown only as counts. No cardholder identity is visible to {merchant.canonical_name} at any point.
          </p>
        </Card>
      </div>
      ...
      <p className="text-[13px] text-ink-secondary mb-4 max-w-2xl">
        Three standalone profiles, not real customers — built to make the {num(segment.size)} legible in five
