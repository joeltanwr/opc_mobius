# Data Generator — OCBC Merchant Dashboard Demo
...
This is the data half of [opc_mobius](../README.md) — the interface that
reads what's produced here lives elsewhere in the repo. Nothing in this
folder depends on the app; it can be regenerated on its own at any time.

Full spec: `mock_data_spec.md`.

## Running it

Both scripts resolve their paths from their own file location, so they work
the same regardless of your current directory — but running them from inside
`data-generator/` (as below) is the convention:

```bash
cd data-generator
python generate.py   # writes data/raw/*.parquet (local) and ../public/data/*.json (repo root, ~100s)
...
`data/raw/` (this folder) is a build artifact — gitignored, ~tens of MB, and
never read by the app. `../public/data/` (repo root) is what actually ships —
currently ~424 KB, under the 600 KB budget. It lives at the repo root rather
than inside this folder because that's the static-assets convention most
frontend tooling (Next.js, Vite, CRA) expects: files under `public/` are
served as-is, so the interface fetches e.g. `/data/taxonomy.json` directly
with no backend in between, matching the spec's "the app never loads raw
transactions, only precomputed aggregates."
...
### `../public/data/` — repo root (JSON — what the app loads)
